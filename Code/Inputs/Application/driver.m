...
c_disc = [];
for i=1:nx*ny
    if ismember(all_indices(i),active_indices)==1
        continue;
    end
    c_disc = [c_disc all_indices(i)];
end
...
Pwf = [];
massbal = [];
flag = 0; %for non physical solution count
...
for curtim = deltim:deltim:endtim
    ...
    for i=1:nx
        for j=1:ny
            ...
            if cell==-1
                A(pos(1),pos(1))=1;
                B(pos(1),1)=-1; %junk pressure value to identify inactive cell
                continue;
            end
    ...
    P1(P1==-1)=NaN;
    S1(S1==-1)=NaN;
    if min(P1)<0 || min(S1)<0 || max(S1)>1
        flag=flag+1;
        disp('Illegal Solution Values!');
    end
